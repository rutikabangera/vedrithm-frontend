...
@Component({
      ...
      <div class="container">
        <div class="quiz-wrapper">
          ...
          <div class="quiz-result" *ngIf="result && !error">
            ...
            <!-- ── CTA with auto WhatsApp message ── -->
  ...
  styles: [`
    ...
    .result-title { font-family:var(--font-display); font-size:clamp(1.5rem,3vw,2.2rem); color:var(--gold); margin-bottom:0.5rem; }
    ...
    @media (max-width:600px) { .quiz-step { padding:1.75rem; } .options-grid { grid-template-columns:1fr; } .reset-btn { margin-left:0; margin-top:0.75rem; } }
  `]
})
export class QuizComponent {
  ...
  answers: { [key: string]: string } = {
    name: '', mobileNumber: '', hairType: '', scalpType: '', primaryConcern: '', lifestyle: ''
  };
  ...
  get allStepIds(): string[] { return ['intro', ...this.steps.map(s => s.id)]; }
  get progressPct(): number { return ((this.currentStep + 1) / this.allStepIds.length) * 100; }
  get currentStepData() { return this.currentStep === 0 ? null : this.steps[this.currentStep - 1]; }
  ...
  submitQuiz() {
    ...
    // Small delay for UX (loading feel), then resolve locally
    setTimeout(() => {
  ...
  reset() {
    ...
    this.error = null; this.mobileError = null;
